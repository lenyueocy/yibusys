...
class Qr_EweiShopV2Page extends WebPage 
{
	...
	public function main() 
	{
		...
		'qr'=>array(
			'title'=>'二维码',
			'url'=>webUrl('platform.qr',array('dos'=>'list')),
			'permission_name'=>'qr',
			'active'=>'list',
		),'display'=>array(
			'title'=>'二维码扫描统计',
			'url'=>webUrl('platform.qr',array('dos'=>'display')),
...
if ($do == 'display') {
	...
	$pager = pagination($total, $pindex, $psize);
		include($this->template('platform/qr/qr-display'));
	...
	iajax(-1, '更新成功!', webUrl('platform/qr',array('dos'=>'display')));
}
	}
	...
	public function post() {
		...
		if (!empty($id)) {
			...
			pdo_update('qrcode', $update, array('uniacid' => $_W['uniacid'], 'id' => $id));
			itoast('恭喜，更新带参数二维码成功！', webUrl('platform/qr'), 'success');
		}
		...
		if (!is_error($result)) {
			...
			pdo_insert('qrcode', $insert);
			itoast('恭喜，生成带参数二维码成功！', webUrl('platform/qr', array('name' => 'qrcode')), 'success');
		} else {
			itoast("公众平台返回接口错误. <br />错误代码为: {$result['errorcode']} <br />错误信息为: {$result['message']}", '', '');
		}
	}
