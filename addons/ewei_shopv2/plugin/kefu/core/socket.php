...
class KefuSocket
{
    ...
    public function checkToken($token = ''){
        global $_W;
        $authkey = $_W['setting']['site']['token'] . '_' . $_W['uniacid'];
        $expire = 3600 * 24 * 30;
        $tokenResult = authcode(base64_decode($token), 'DECODE', $authkey, $expire);
        if(empty($tokenResult)) return false;
	...
	public function onMessage($server, $data, $fd)
	{
		...
		if (empty($data['type']) || empty($data['token']) || !$this->checkToken($data['token'])) {
			return false;
		}
        $this->log('member',$this->member);
        $this->roomid = $data['role']=='kefu'?$data['roomid']:$this->mid;
		if ($data['type'] == 'login') {
			$addData = array(
				'fd' => $fd,
				'mid' => $this->mid,
				...
				'roomid' => $this->roomid,
				'role' => $data['role']=='kefu'?'kefu':'user'
			);
            $this->addUser($server, $this->mid, $addData);
			//$banned = $this->getBanned($server, $data['roomid'], $this->mid);
			//$this->log('echo',json_encode($banned));
			...
			$sendArr = array(
				'pages'=>'message_service.msgList',
				'type'=>'push',
				'data'=>[
                    'type' => 'system',
                    'msg'=>[
                        'id' => $this->getMsgid($this->mid),
                        'type' => 'text',
			...
			if ($data['role'] == 'kefu') {
				...
				//如果是客服，那么进入房间 推送一条消息给用户提示 是哪位客服在线服务
                $this->kefuData = $kefuData = pdo_get('ewei_kefu',['openid'=>$this->openid]);
                $this->log('user',print_r($kefuData,1));
                $sendArr = array(
                    'pages'=>'message_service.msgList',
                    'type'=>'push',
                    'data'=>[
                        ...
                        'msg'=>[
                            'id' => $this->getMsgid($this->mid),
                            'type' => 'text',
                            'content' => ['text'=>$kefuData['name'].'为您服务!'],
                        ]
                    ],
                );
                $this->sendAll($server, $sendArr, $data['roomid'],'user');
			}
        ...
        $table_records = $this->getTable('chat_records', $this->roomid);
		if ($data['type'] == 'update') {
			$this->updateUser($data['uid'], array());
		} else {
			if ($data['type'] == 'text' || $data['type'] == 'img' || $data['type'] == 'voice') {
				//用户发送信息 则推送给所有房间的用户
                $table_records = $this->getTable('chat_records', $this->roomid);
                $chat_records = [
                    'type' => 'user',
                    'msg'=>[
                        ...
                        'userinfo'=>[
                            'uid'=>$this->mid,
                            'username'=>$data['role']=='kefu'?$this->kefuData['name']:$this->member['nickname'],
                            'face'=>$data['role']=='kefu'?$this->kefuData['avatar']:$this->member['avatar']
                        ],
                        'content' => $data['content'],
                    ]
				];
                $this->redis($server)->rPush($table_records,json_encode($chat_records));
                $rlength = $this->redis($server)->lLen($table_records);
                if (77 < $rlength) $this->redis($server)->lTrim($table_records, $rlength - 77, -1);
                $sendArr = array(
                    'pages'=>'message_service.msgList',
                    'type'=>'push',
                    'data'=>[
                ...
                $this->sendAll($server,$sendArr,$this->roomid);
			}else if($data['type'] == 'record'){
				if(!empty($data['content']) && !empty($data['content']['msg'])){
					//查出所有数据 然后根据 msg_id 来切割
                    $rlength = $this->redis($server)->lLen($table_records);
                    $msgResultAll = redis()->lrange($table_records,0,-1);
                    $msgResultAll = array_reverse($msgResultAll);
                    foreach ($msgResultAll as $key=>&$val){
                        $val = json_decode($val,true);
                    }
                    $msgResult = array_column(array_column($msgResultAll,'msg'),'id');
                    $msgIndex = array_search($data['content']['msg']['id'],$msgResult);
                    $msgResult = array_slice($msgResultAll,$msgIndex+1,20);
                    if(empty($msgResult)) {
                        $sendArr = array(
                            'pages'=>'message_service.loadFinish',
                            'type'=>'replace',
                            'data'=>true,
                        );
                        $this->send($server, $fd, $sendArr);
                    ...
                    $sendArr = array(
                        'pages'=>'message_service.msgList',
                        'type'=>'reverse',
                        'data'=>array_reverse($msgResult),
                    );
                    $this->send($server, $fd, $sendArr);
				}else{
					//初始化加载20条记录
                    $rlength = $this->redis($server)->lLen($table_records);
                    $lrangeList = $this->redis($server)->lrange($table_records,$rlength - 20,-1);
                    foreach ($lrangeList as $key=>&$val) $val = json_decode($val,true);
                    $sendArr = array(
                        'pages'=>'message_service.msgList',
                        'type'=>'reverse',
                        'data'=>$lrangeList,
	...
	protected function sendAll($server = NULL, $data = array(), $roomid = 0,$allUserType = false, $fd = 0)
	{
		if (empty($server) || empty($data) || empty($roomid)) {
			return false;
		}

		$allUser = $this->getAllUser($server, $roomid);
		$this->log('user',print_r($allUser,1));
		...
		foreach ($allUser as $uid => $user) {
            ...
            if($allUserType && $user['role'] != $allUserType) continue;
