...
class Pay_EweiShopV2Page extends AppMobilePage
{
	...
	public function main()
	{
		...
		if (!empty($set['pay']['wxapp']) && 0 < $order['price'] && $this->iswxapp) {
			...
			$payinfo = array('openid' => $member['openid_wa'], 'title' => $set['shop']['name'] . '订单', 'tid' => $tid, 'fee' => $order['price']);
	...
	public function result(){
		...
		$list = pdo_get('ewei_shop_order',['id'=>$_GPC['id'],'openid'=>$_W['openid']],['id','status']);
