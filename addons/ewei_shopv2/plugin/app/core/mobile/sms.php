...
class Sms_EweiShopV2Page extends AppMobilePage
{
	...
	protected function loginSMS($temp)
	{
		...
		if (!empty($wapset['smsimgcode']) && 0) {
			if (empty($verifyImgCode)) {
				return app_error(AppError::$ParamsError,'请输入图形验证码');
			}

			$verifyCodeKey = 'sms_captcha_code_uniaicid_' . $_W['uniacid'] . '_mobile_' . $_GPC['mobile'];
	...
	public function captcha()
	{
		...
		m('cache')->set('sms_captcha_code_uniaicid_' . $_W['uniacid'] . '_mobile_' . $_GPC['mobile'], $hash);
		$captcha->output();
	}
}
