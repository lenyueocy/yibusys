...
class Cart_EweiShopV2Page extends AppMobilePage
{
	...
	//获取猜你喜欢商品 Guess like
	public function getGuessLikeGoods(){
		global $_W;
		global $_GPC;
		//$guessLikeGoods = pdo_getall('ewei_shop_goods');
		$guessLikeGoods = m("goods")->getList([]);
		return app_json($guessLikeGoods);
	}
	...
	public function selectedStatus(){
		...
		$table_cart = tablename('ewei_shop_member_cart');
		$sql = "update {$table_cart} set selected={$selected} where id in ({$_GPC['ids']})";
		pdo_query($sql);
		//$this->cartGoodsCheckedStatus($_GPC['ids'],intval($_GPC['selected']));
		return app_json(0);
	}
