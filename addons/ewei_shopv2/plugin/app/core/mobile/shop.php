...
class Shop_EweiShopV2Page extends AppMobilePage
{
    ...
    public function get_shopindex()
	{
		...
		$category = pdo_getall('ewei_shop_category',['ishome'=>1,'enabled'=>1],[],'',['displayorder desc']);
		foreach ($category as $key=>&$val){
			//$goods = pdo_getall('ewei_shop_goods',['cates'=>$val['id']]);
			$goods = m('goods')->getList(['cate'=>$val['id'],'pagesize'=>12]);
			if(!empty($goods) && !empty($goods['list'])) $val['goods'] = $goods['list'];
		}
		$result = [
			'advs'=>$advs,
			'category'=>$category,
			'recommands'=>$indexrecommands,
		];
	...
	public function get_category()
	{
		...
		//$bList = pdo_getall('ewei_shop_brand',[],[],'',['CONVERT(NAME USING gbk)'=>'asc']);
//		$table_brand = tablename('ewei_shop_brand');
//		$where = " status=1 and is_delete=0 and uniacid={$_W['uniacid']} ";
//		if(!empty($_GPC['cate_id'])) $where .= " and cate_id={$_GPC['cate_id']} ";
//		$bList = pdo_fetchall("select * from {$table_brand} where {$where} order by CONVERT(NAME USING gbk) asc");
//		foreach ($bList as $key=>$val){
//			$firstLetter = m('shop')->getPinyin($val['name'],'one');
//			if((ord($firstLetter)>=ord('A') && ord($firstLetter)<=ord('Z'))||(ord($firstLetter)>=ord('a') && ord($firstLetter)<=ord('z'))) $letter = strtoupper($firstLetter);
//			else $letter = 'well';
//			if(empty($brandList[$letter]['letter'])) $brandList[$letter]['letter'] = $letter;
//			$val['logo'] = tomedia($val['logo']);
//			$brandList[$letter]['data'][] = $val;
//		}
//		$wellArr = $brandList['well'];
//		unset($brandList['well']);
//		$brandList = array_values($brandList);
//		$brandList[] = $wellArr;
//		$brandCategoryList = pdo_getall('ewei_shop_brand_category',['uniacid'=>1,'status'=>1],[],'',['displayorder desc']);
//		array_unshift($brandCategoryList,['id'=>0,'name'=>'全部品牌']);
