...
function cloud_m_prepare($name) {
	...
	$pars['module'] = $name;
	$dat = cloud_request('http://cloud.wxapp.hk/gateway.php', $pars);
	if (is_error($dat)) {
		return $dat;
	}
	return true;
...
function cloud_sms_send($mobile, $content, $postdata = array()) {
	...
	$response = cloud_request('http://cloud.wxapp.hk/gateway.php', $pars);
...
function cloud_download($path, $type = '') {
	...
	$dat = cloud_request('http://cloud.wxapp.hk/gateway.php', $pars, $headers, 300);
	if(is_error($dat)) {
...
function _cloud_shipping_parse($dat, $file) {
	...
	if ($dat['content'] == 'blacklist') {
		return error(-1, '抱歉，您的站点已被列入新睿云服务黑名单，新睿云服务一切业务已被禁止，请联系新睿社区客服！');
	}
...
function cloud_cron_create($cron) {
	...
	$result = cloud_request('http://cloud.wxapp.hk/gateway.php', $pars);
...
function cloud_auth_ip(){
	...
	$pars['cron_id'] = $cron_id;
	$pars['status'] = $status;
	$result = cloud_request('http://cloud.wxapp.hk/gateway.php/a=ipcl', $pars);
	...
	$auth_url = 'http://cloud.wxapp.hk/gateway.php/a=ipcl&__auth=' . $query;
...
function cloud_auth_url($forward, $data = array()){
	...
	$query = base64_encode(json_encode($auth));
	$auth_url = 'http://cloud.wxapp.hk/gateway.php/a=passwort&__auth=' . $query;
...
function cloud_flow_master_post($flow_master) {
	...
	$dat = cloud_request('http://cloud.wxapp.hk/gateway.php', $pars, array(), 300);
	if(is_error($dat)) {
		return error(-1, '网络存在错误， 请稍后重试。' . $dat['message']);
	}
