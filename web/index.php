<?php
if(!isset($_GET['c'])&&!isset($_GET['e'])&&!isset($_GET['m'])){
    if(!isset($_GET['c'])){	$_GET['c']='site'; }
    if(!isset($_GET['a'])){	$_GET['a']='entry'; }
    if(!isset($_GET['m'])){	$_GET['m']='ewei_shopv2'; }
    if(!isset($_GET['do'])){	$_GET['do']='web'; }
}
...
if ($_W['role'] != ACCOUNT_MANAGE_NAME_FOUNDER) {
	...
	if (in_array(FRAME, array('system', 'site', 'account_manage', 'platform', 'module'))) {
