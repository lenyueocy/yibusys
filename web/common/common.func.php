		...
		if (!empty($module['plugin_list']) && empty($module['main_module'])) {
			...
			if (!empty($plugin_list)) {
				...
				foreach ($plugin_list as $plugin_module) {
					...
					$frames['account']['section']['platform_module_plugin']['menu']['platform_' . $plugin_module_info['name']] = array(
						...
						'url' => url('account/display/switch', array('module_name' => $plugin_module_info['name'], 'uniacid' => $_W['uniacid'], 'version_id' => $version_id)),
						'is_display' => 1,
					);
					$i++;
				}
			...
			$frames['account']['section']['platform_module_plugin']['menu']['platform_module_plugin_more'] = array(
				...
				'is_display' => empty($module['main_module']) ? 1 : 0,
						...
						if ($_W['account']->supportVersion && count($version_modules) <= 1) {
				$frames['account']['section']['platform_module_plugin'] = false;
				$frames['account']['section']['platform_module_plugin']['menu']['platform_module_plugin_more']['is_display'] = false;
			}
		}
