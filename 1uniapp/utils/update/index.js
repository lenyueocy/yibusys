...
// 主颜色
const $mainColor = "e41f20";
// 弹窗图标url
const $iconUrl = "/static/icon/update/ic_ar.png";
...
export const getServerNo = function(version,isPrompt = false, callback) {
	 ...
	 */ 
	$http.get("update", httpData,{ 
		isPrompt: isPrompt,load:false,loadbar:false
	}).then(res => {
...
function updatePopup(data, callback) {
 ...
 if(!data.versionInfo){
	  ...
	  if (data.update && data.downloadUrl) {  
			 uni.downloadFile({  
