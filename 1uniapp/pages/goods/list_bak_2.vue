<template>
	<view class="container">
		...
		<view class="tui-header-box">
			<view class="tui-header" :style="{ width: width + 'px', height: height + 'px' }">
				...
				<view class="tui-searchbox tui-search-mr" :style="{ marginTop: inputTop + 'px' }" @tap="search">
					...
					<text class="tui-search-text" v-if="!keyword">搜索</text>
					<view class="tui-search-key" v-if="keyword"><!--v-if="searchKey"-->
						<view class="tui-key-text">{{keyword}}</view>
						<tui-icon name="shut" :size="12" color="#fff" @click="searchClose"></tui-icon>
					</view>
				</view>
			</view>
		</view>
		...
		<view class="tui-header-screen" :style="{ top: height + 'px' }">
			<view class="tui-screen-top">
				...
				<view class="tui-dropdownlist" :class="[selectH > 0 ? 'tui-dropdownlist-show' : '']" :style="{ height: selectH + 'rpx' }">
					<view
						...
						:class="[item.selected ? 'tui-bold' : '',item.selected?'tui-active':'']"
			...
			<view class="tui-screen-bottom" v-if="0">
		...
		<mescroll-body class="" ref="mescrollRef" @init="mescrollInit" @down="downCallback" @up="upCallback" >
			<view class="tui-product-list" :class="{'padding-lr-sm':!isList}" :style="{ marginTop: px(dropScreenH + 25) }">
				<view class="tui-product-container">
					<block v-for="(item, index) in goodsList" :key="index" v-if="(index + 1) % 2 != 0 || isList">
						<!--商品列表-->
						<view @click="navTo('/pages/goods/detail',{id:item.id})" class="tui-pro-item" :class="[isList?'tui-flex-list':'',!isList?'border-radius-list':'']" hover-class="hover" :hover-start-time="150">
							<view style="position: relative;">
                                <image :src="item.thumb" class="tui-pro-img" :class="[isList ? 'tui-proimg-list' : '']" mode="widthFix" />
                                <view v-if="!isList" class="flex justify-end" style="width:100%;position:absolute;bottom:10rpx;right:0rpx;">
                                    <view v-if="item.isrecommand" class="text-white text-xs margin-right-xs" style="padding:6rpx 10rpx;background-color:#F8B600;border-radius: 20rpx 0 20rpx 0;">推荐</view>
                                    <view v-if="item.isnew" class="text-white text-xs margin-right-xs" style="padding:6rpx 10rpx;background-color:#F8B600;border-radius: 20rpx 0 20rpx 0;">新品</view>
                                    <view v-if="item.ishot" class="text-white text-xs margin-right-xs" style="padding:6rpx 10rpx;background-color:#F8B600;border-radius: 20rpx 0 20rpx 0;">热卖</view>
                                </view>
                            </view>
                            <view class="tui-pro-content">
								<view class="tui-pro-tit">{{item.title}}</view>
                                <view class="flex" v-if="isList">
                                    <view v-if="item.isrecommand" class="text-white text-xs margin-right-xs" style="padding:6rpx 10rpx;background-color:#F8B600;border-radius: 20rpx 0 20rpx 0;">推荐</view>
                                    <view v-if="item.isnew" class="text-white text-xs margin-right-xs" style="padding:6rpx 10rpx;background-color:#F8B600;border-radius: 20rpx 0 20rpx 0;">新品</view>
                                    <view v-if="item.ishot" class="text-white text-xs margin-right-xs" style="padding:6rpx 10rpx;background-color:#F8B600;border-radius: 20rpx 0 20rpx 0;">热卖</view>
                                </view>
								<view>
									<view class="tui-pro-price flex align-end justify-between">
										<!--<text class="tui-sale-price">￥{{parseInt(item.hasoption)==1?item.productprice:item.minprice}}</text>-->
										<text class="tui-sale-price">￥{{item.minprice}}</text>
                                        <image src="/static/images/goods/list/add-cart-btn.png" style="width: 60rpx;height: 60rpx;" />
			...
			<view class="tui-drawer-box" :style="{ paddingTop: height + 'px' }">
				<scroll-view class="tui-drawer-scroll" scroll-y :style="{ height: drawerH + 'px'}">
					...
					<view class="tui-drawer-content">
						<input placeholder-class="tui-phcolor" class="tui-input" placeholder="最低价" maxlength="11" type="number" v-model="filters.minprice" />
						<tui-icon name="reduce" color="#333" :size="14"></tui-icon>
						<input placeholder-class="tui-phcolor" class="tui-input" placeholder="最高价" maxlength="11" type="number" v-model="filters.maxprice" />
					</view>
					...
					<view class="tui-drawer-content tui-flex-attr"><!--tui-btmItem-active-->
						<view @click="filters.cate=item.id" class="tui-attr-item" :class="[filters.cate==item.id?'tui-btmItem-active':'']" v-for="(item,index) in categoryList" :key="index">
				...
				<view class="tui-attr-btnbox">
					<view class="tui-attr-safearea">
						<view class="tui-drawer-btn tui-drawerbtn-black" hover-class="tui-white-hover" :hover-stay-time="150" @tap="resetFilters">重置</view>
						<view class="tui-drawer-btn tui-drawerbtn-primary" hover-class="tui-red-hover" :hover-stay-time="150" @tap="submitFilters">确定</view>
					</view>
				</view>
			</view>
		</tui-drawer>
	...
	export default {
		...
		data() {
			return {
				...
				goodsList: [],
                categoryList: [],
                filters:{minprice:'',maxprice:'',cate:''},/*minprice:'',maxprice:'',cate:''*/
			};
		},
		onLoad: function(options) {
			...
			uni.getSystemInfo({
				success: res => {
					...
					//略小，避免误差带来的影响
					this.dropScreenH = (this.height * 750) / res.windowWidth + 90;//原+186
					this.drawerH = res.windowHeight - uni.upx2px(100) - this.height;
				}
			});
			if(this.$Route.query.cate_name) this.keyword = this.$Route.query.cate_name
			if(this.$Route.query.brand_name) this.keyword = this.$Route.query.brand_name
			if(this.$Route.query.type == 'ishot') this.attrArr[1].isActive = true
		...
		methods: {
			getList:function () {
                ...
                if(this.$Route.query.cate_id) params.cate = this.$Route.query.cate_id
				...
				if(this.tabIndex==1){
                    ...
                    this.dropdownList.map((item,index)=>{
                        ...
                        if(item.selected && index==2){
                            params.order = 'minprice'
                            params.by = 'desc'
                        }
                    })
				}
				this.$http.get('goods.get_list',params).then((res)=>{
					this.mescroll.endBySize(this.mescroll.size, parseInt(res.total));
					if(this.mescroll.num == 1) this.goodsList = [];
					if(res.list) this.goodsList = this.goodsList.concat(res.list)
				})
			},
			getCategoryList:function () {
                this.$http.get('goods.get_category').then((res)=>{
            ...
            resetFilters:function () {
                this.mescroll.resetUpScroll()
				this.filters = {minprice:'',maxprice:'',cate:''}
                this.getList()
                this.drawer = false
            },
			...
			dropdownItem: function(e) {
                ...
                this.dropdownList.map((item,index)=>{
                    if (parseInt(clickIndex) == index) item.selected = true
					else item.selected = false
				})
				this.selectedName = clickIndex == 0 ? '综合' : '价格';
				this.selectH = 0;
				this.downCallback()
			},
			screen: function(e) {
