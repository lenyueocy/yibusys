<template>
	<view class="container">
		...
		<view class="tui-header-box">
			<view class="tui-header" :style="{ width: width + 'px', height: height + 'px' }">
				...
				<view class="tui-searchbox tui-search-mr" :style="{ marginTop: inputTop + 'px' }" @tap="search">
					...
					<text class="tui-search-text" v-if="!filters.keyword">搜索</text>
					<view class="tui-search-key" v-if="filters.keyword"><!--v-if="searchKey"-->
						<view class="tui-key-text">{{filters.keyword}}</view>
		...
		<mescroll-body class="" ref="mescrollRef" @init="mescrollInit" @down="downCallback" @up="upCallback" >
			<view class="tui-product-list" :class="[!isList?'padding-lr-sm':'padding-lr-sm']" :style="{ marginTop: px(dropScreenH + 25) }">
                            ...
                            <view class="tui-pro-content">
								...
								<view>
									<view class="tui-pro-price ">
										...
										<text class="tui-sale-price text-price">{{item.minprice}}</text>
                                        <text class="tui-factory-price">{{ item.sales }}人付款</text>
	...
	export default {
			...
			return {
                ...
                attrArr: [
                    /*{
                        ...
                        isActive: false,
                        key:'isnew',
                        list: [{name: 'trendsetter',selected: false}]
                    },*/
                    {
                        name: '新品',
                        selectedName: '新品',
				...
				goodsList: [],
                categoryList: [],
                filters:{page:1,pagesize:10,keyword:'',minprice:'',maxprice:'',cate:''},/*minprice:'',maxprice:'',cate:''*/
			};
		},
		onLoad: function(options) {
			...
			uni.getSystemInfo({
				success: res => {
					...
					//略小，避免误差带来的影响
					this.dropScreenH = (this.height * 750) / res.windowWidth + 186;//原+90
					this.drawerH = res.windowHeight - uni.upx2px(100) - this.height;
				}
			});
            if(this.$Route.query.cate_id) this.filters.cate = this.$Route.query.cate_id
		...
		methods: {
			getList:function () {
                ...
                params.page = this.mescroll.num||1
                params.pagesize = this.mescroll.size
			    if(this.$Route.query.keyword) params.keywords = this.$Route.query.keyword
			...
			getCategoryList:function () {
                this.$http.get('goods.get_category').then((res)=>{
					if(res.allcategory && res.allcategory.parent) this.categoryList = res.allcategory.parent
                })
            },
