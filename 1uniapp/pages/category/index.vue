		...
		<block v-for="(item, index) in dataList" :key="index">
			<scroll-view scroll-y class="right-box" :style="{ height: height + 'px', top: top + 'px' }" v-if="currentTab == index">
				...
				<view class="page-view">
                    ...
                    <swiper indicator-dots autoplay circular :interval="5000" :duration="150" class="swiper margin-bottom-sml" v-if="item.advimg">
                        <swiper-item @tap.stop="$Router.push({path:item.advurl})" >
                            <image :src="item.advimg" class="slide-image" />
                        </swiper-item>
                    </swiper>

					<view class="class-box" >
						<view class="class-item bg-white" >
							<view class="class-name">{{item.name}}</view>
							<view class="g-container" v-if="item.goods && item.goods.length>0">
                                <view class="g-box" @tap.stop="$Router.push({name:'goods_detail',params:{id:e.id}})" v-for="(e,i) in item.goods">
                                    <image :src="e.thumb" class="g-image border-10" />
                                    <view class="g-title">{{e.title}}</view>
                                    <view class="sale-price text-center">￥{{e.marketprice}}</view>
                                </view>
							</view>
                            <view v-else class="text-gray text-sm padding-lg text-center">
                                <tui-icon name="nodata" :size="40"></tui-icon>
                                <view class="margin-top-xs">无更多商品</view>
                            </view>
                            <view @click="$Router.push({name:'goods_list',params:{cate_id:item.id}})" class="flex align-center justify-center padding-top-sm" style="color: #4a4a4a;">
                                <view class="text-sml">发现更多{{item.name}}</view>
