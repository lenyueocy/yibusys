	...
	<view class="container">
		<view class="tui-order-header">
			<view class="tui-text">订单编号：
				<text class="tui-bold">{{data.ordersn}}</text>
			</view>
			<view class="tui-text">快递公司：<text class="">{{data.expresscom}}</text></view>
			<view class="tui-text">快递单号：<text class="">{{data.expresssn}}</text></view>
		</view>
		<view class="tui-order-tracking">
			<view v-if="expressData">
				<tui-time-axis>
					...
					<!--<tui-timeaxis-item backgroundColor="transparent">
						...
						<tui-timeaxis-item backgroundColor="transparent" v-for="(item,index) in expressData.Traces" :key="index">
							<template v-slot:node>
							...
							<template v-slot:content>
								<view class="tui-order-title" :class="[index==0?'text-black':'text-gray']" v-if="index==0 && expressData.State==3">已签收</view>
								...
								<view class="tui-order-time text-gray">{{item.AcceptTime}}</view>
							</template>
						</tui-timeaxis-item>
				</tui-time-axis>
			</view>
	...
	export default {
		...
		methods:{
		    getList:function () {
				...
				this.$http.get('util.express',{express:this.$Route.query.express,expresssn:this.$Route.query.expresssn}).then((res)=>{
					this.expressData = res.express
				})
            }
		}
	}
