﻿<template>
	<view class="app">
		...
		<tui-list-view unlined="all" class="pay-type-list" title="请选择支付方式" style="background-color: #f6f6f6;" >
			...
			<tui-list-cell v-if="payInfo.alipay.success ||1" @click="changePayType('alipay')" class="type-item b-b" :arrow="false" padding="30rpx 50rpx">
			...
			<tui-list-cell v-if="payInfo.wechat.success||1" @click="changePayType('wechat')" class="type-item b-b" :arrow="false" padding="30rpx 50rpx">
...
<script>
    ...
    export default {
        ...
        methods: {
            getList(){
                this.$http.get('appoint.pay',{id:this.$Route.query.orderid}).then((res)=>{
					this.payInfo = res
            ...
            submit(){
                ...
                switch (this.payType){
                    case 'credit':
                        this.$http.get('appoint.pay.complete',{id:this.orderInfo.id,type:this.payType}).then((res)=>{
                            if(res.error==0){
								this.$Router.push({name:'order_pay_result'})
                            }else{
